...
describe('ProductController', () => {
  ...
  describe('updateProduct', () => {
    ...
    it('should delete old image if productImage is explicitly set to null/empty in body', async () => {
      mockRequest.params = { id: '1' };
      mockRequest.body = { productImage: null, name: 'Name with no image' }; // Explicitly clearing image
      const updatedProductResult = { ...existingProductWithImage, productImage: null, name: 'Name with no image' };
    ...
    it('should not delete old image if no new image is provided and no explicit null/empty', async () => {
      mockRequest.params = { id: '1' };
      mockRequest.body = { name: 'Only Name Updated' }; // Only updating name, no productImage field
      const updatedProductResult = { ...existingProductWithImage, name: 'Only Name Updated' };

      mockProductRepository.findProductById.mockResolvedValue(existingProductWithImage as any);
      mockCloudinaryUtils.uploadFile.mockResolvedValue(''); // No new image to upload
      // mockCloudinaryUtils.deleteFile.mockResolvedValue(undefined); // This should not be called
      mockProductRepository.updateProduct.mockResolvedValue(updatedProductResult as any);
      ...
      expect(mockProductRepository.updateProduct).toHaveBeenCalledWith(1, expect.objectContaining({
        ...
        productImage: existingProductWithImage.productImage // Should retain old image
      }));
      expect(mockResponse.status).toHaveBeenCalledWith(200);
      expect(mockResponse.json).toHaveBeenCalledWith(updatedProductResult);
    });
    ...
    it('should return 500 if an error occurs during old image deletion', async () => {
      ...
      mockProductRepository.updateProduct.mockResolvedValue({} as any); // Still expect update to be called

      await ProductController.updateProduct(mockRequest as Request, mockResponse as Response);

      expect(mockResponse.status).toHaveBeenCalledWith(500);
      expect(mockResponse.json).toHaveBeenCalledWith({ message: errorMessage });
      expect(mockProductRepository.updateProduct).toHaveBeenCalled(); // It should proceed
    });

    it('should return 500 if an error occurs during product update in repository', async () => {
      mockRequest.params = { id: '1' };
      mockRequest.body = { name: 'Update' };
      const errorMessage = 'Repository update failed';

      mockProductRepository.findProductById.mockResolvedValue(existingProductWithImage as any);
      mockProductRepository.updateProduct.mockRejectedValue(new Error(errorMessage));
